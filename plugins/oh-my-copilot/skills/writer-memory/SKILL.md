...
### With omcp Notepad / Memory Tools
Writer Memory integrates with omcp's notepad and project memory tools:
- Scene ideas can be captured as notes and persisted via project memory tools
- Character insights from analysis sessions are preserved across sessions
- Cross-reference between notepad entries and writer-memory records

### With Architect Agent
For complex character analysis, dispatch a subagent through `/fleet`:
```
/fleet architect --model=opus --prompt="Analyze 새랑's arc across all scenes..."
```
