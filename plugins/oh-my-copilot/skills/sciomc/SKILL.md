...
Fire independent stages in parallel by dispatching subagents through `/fleet`:

```
# Stage 1 - Simple data gathering
/fleet scientist --model=haiku --prompt="[RESEARCH_STAGE:1] Investigate..."

# Stage 2 - Standard analysis
/fleet scientist --model=sonnet --prompt="[RESEARCH_STAGE:2] Analyze..."

# Stage 3 - Complex reasoning
/fleet scientist --model=opus --prompt="[RESEARCH_STAGE:3] Deep analysis of..."
```

Use `/delegate` instead of `/fleet` when a stage should run on a GitHub-hosted runner rather than a local parallel worker.

### Smart Model Routing

**CRITICAL: Always pass `--model` parameter explicitly!**

| Task Complexity | Agent | Claude Model | GPT Model | Use For |
|-----------------|-------|--------------|-----------|---------|
| Data gathering | `scientist` | haiku | gpt-5-mini | File enumeration, pattern counting, simple lookups |
| Standard analysis | `scientist` | sonnet | gpt-5.2-codex | Code analysis, pattern detection, documentation review |
| Complex reasoning | `scientist` | opus | gpt-5.4 | Architecture analysis, cross-cutting concerns, hypothesis validation |
...
/fleet scientist --model=sonnet --prompt="
...
[Execution - Parallel]
Firing stages 1-3 in parallel through `/fleet`...
...
```
# All fire simultaneously through `/fleet`
/fleet scientist --model=haiku --prompt="[STAGE:1] Analyze src/api/..."
/fleet scientist --model=haiku --prompt="[STAGE:2] Analyze src/utils/..."
/fleet scientist --model=haiku --prompt="[STAGE:3] Analyze src/components/..."
```
...
/fleet scientist --model=sonnet --prompt="[HYPOTHESIS:A] Test if caching improves..."
/fleet scientist --model=sonnet --prompt="[HYPOTHESIS:B] Test if batching reduces..."
/fleet scientist --model=sonnet --prompt="[HYPOTHESIS:C] Test if lazy loading helps..."
```
...
/fleet scientist --model=opus --prompt="
[CROSS_VALIDATION]
