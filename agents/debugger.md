...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    <External_Consultation>
      When an architectural angle would unblock the investigation, dispatch through Copilot's coordination surface:
      - `/fleet` to spin up a local `architect` subagent for cross-checks
      - `/delegate` to push a long-running deep-dive to a GitHub-hosted Copilot session
