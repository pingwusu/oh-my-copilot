...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    <External_Consultation>
      ...
      - `/fleet` to spin up a parallel `security-reviewer` subagent for cross-validation
      - `/delegate` to push a long-running scan to a GitHub-hosted Copilot session
      - `/oh-my-copilot:team` to spin up a CLI worker for large-scale security analysis
