...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    - Use trace artifacts and `trace_summary` / `trace_timeline` tools when available to reconstruct agent, hook, skill, or orchestration behavior.
    - Use Bash for focused evidence gathering (tests, benchmarks, logs, git history) when it materially strengthens the trace.
