---
name: team
description: N coordinated agents on shared task list using Copilot CLI's parallel dispatch surface
aliases: []
level: 4
model:
...
Spawn N coordinated agents working on a shared task list using Copilot CLI's parallel dispatch surface (`/fleet`, `/delegate`, `/tasks`). Replaces the legacy `/swarm` skill (SQLite-based) with built-in team management, inter-agent messaging, and task dependencies -- no external dependencies required.

The `swarm` compatibility alias is not carried over to omcp.
...
User: "/team 3:executor fix all TypeScript errors"
      ...
      [TEAM ORCHESTRATOR (Lead)]
              |
              +-- Create team handle ("fix-ts-errors")
              |       -> lead becomes team-lead@fix-ts-errors
              |
              +-- Analyze & decompose task into subtasks
              |       -> explore/architect produces subtask list
              |
              +-- Create N entries in `/tasks` (one per subtask)
              |       -> tasks #1, #2, #3 with dependencies
              |
              +-- Pre-assign owners on the `/tasks` board
              |       -> task #1 owner=worker-1, etc.
              |
              +-- Dispatch N subagents through `/fleet` (worker-1...worker-N)
              |       -> spawns teammates into the team
              |
              +-- Monitor loop
              |       <- inbound messages from teammates (auto-delivered)
              |       -> poll `/tasks` for progress
              |       -> send messages to unblock/coordinate
              |
              +-- Completion
                      -> send shutdown_request to each teammate
                      <- receive shutdown_response (approve: true)
                      -> tear down the team handle
...
|-------|----------------|-----------------|-------------------|
| **team-plan** | `explore` (haiku / gpt-5-mini), `planner` (opus / gpt-5.4) | `analyst` (opus / gpt-5.4), `architect` (opus / gpt-5.4) | Use `analyst` for unclear requirements. Use `architect` for systems with complex boundaries. |
| **team-prd** | `analyst` (opus / gpt-5.4) | `critic` (opus / gpt-5.4) | Use `critic` to challenge scope. |
| **team-exec** | `executor` (sonnet / gpt-5.2-codex) | `executor` (opus / gpt-5.4), `debugger` (sonnet / gpt-5.2-codex), `designer` (sonnet / gpt-5.2-codex), `writer` (haiku / gpt-5-mini), `test-engineer` (sonnet / gpt-5.2-codex) | Match agent to subtask type. Use `executor` (model=opus/gpt-5.4) for complex autonomous work, `designer` for UI, `debugger` for compilation issues, `writer` for docs, `test-engineer` for test creation. |
| **team-verify** | `verifier` (sonnet / gpt-5.2-codex) | `test-engineer` (sonnet / gpt-5.2-codex), `security-reviewer` (sonnet / gpt-5.2-codex), `code-reviewer` (opus / gpt-5.4) | Always run `verifier`. Add `security-reviewer` for auth/crypto changes. Add `code-reviewer` for >20 files or architectural changes. `code-reviewer` also covers style/formatting checks. |
| **team-fix** | `executor` (sonnet / gpt-5.2-codex) | `debugger` (sonnet / gpt-5.2-codex), `executor` (opus / gpt-5.4) | Use `debugger` for type/build errors and regression isolation. Use `executor` (model=opus/gpt-5.4) for complex multi-file fixes. |
...
2. **Specialist agents complement executor agents.** Route analysis/review to architect/critic Claude/GPT agents and UI work to designer agents. Tmux CLI workers are one-shot and don't participate in team communication.
3. **Cost mode affects model tier.** In downgrade: top-tier agents (opus / gpt-5.4) to mid-tier (sonnet / gpt-5.2-codex), mid-tier to low-tier (haiku / gpt-5-mini) where quality permits. `team-verify` always uses at least the mid tier.
4. **Risk level escalates review.** Security-sensitive or >20 file changes must include `security-reviewer` + `code-reviewer` (top tier) in `team-verify`.
...
- **team-exec**
  - Entry: team handle created, `/tasks` populated, assignment, and worker spawn are complete.
...
3. **On team cancellation, handoffs survive** in `.omcp/handoffs/` for session resume. They are not deleted when the team handle is torn down.
...
- **Resume:** restart from the last non-terminal stage using staged state + live `/tasks` status. Read `.omcp/handoffs/` to recover stage transition context.
- **Cancel:** `/oh-my-copilot:cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then tears down the team handle and clears/preserves team state per policy. Handoff files in `.omcp/handoffs/` are preserved for potential resume.
...
Use `explore` or `architect` (dispatched through `/fleet`) to analyze the codebase and break the task into N subtasks:
...
Create a team handle with a slug derived from the task (e.g., `fix-ts-errors`). The omcp runtime tracks this in `~/.copilot/teams/<slug>/config.json` and assigns the current session as `team-lead@<slug>`.
...
Create one `/tasks` entry per subtask. Set dependencies by updating the `blockedBy` field on dependent tasks.

```json
// New task entry for subtask 1
{
...
For tasks with dependencies, update after creation:
...
- Internal tasks appear in the `/tasks` listing -- filter them when counting real tasks
...
1. **Inbound messages** -- Teammates send messages to `team-lead` when they complete tasks or need help. These arrive automatically as new conversation turns (no polling needed).

2. **`/tasks` polling** -- Periodically inspect the `/tasks` board to check overall progress:
   ```
...
- **Reassign work:** If a teammate finishes early, update the `/tasks` entry to assign pending tasks to them and notify via message
...
1. **Verify results** -- Check that all subtasks are marked `completed` on the `/tasks` board
...
4. **Tear down team handle** -- Remove the team via the omcp runtime:
   ```json
   { "team_name": "fix-ts-errors" }
   ```
   Expected response:
...
Check the `/tasks` board periodically to see if blockers have been resolved.
...
== RULES ==
- NEVER dispatch sub-agents or use `/fleet`
- NEVER run tmux pane/session orchestration commands (for example `tmux split-window`, `tmux new-session`)
- NEVER run team spawning/orchestration skills or commands (for example `$team`, `$ultrawork`, `$autopilot`, `$ralph`, `omcp team ...`)
- ALWAYS use absolute file paths
- ALWAYS report progress via messages to "team-lead"
- Use messages with type "message" only -- never "broadcast"
```
...
- `claude_worker` / `gpt_worker`: Emphasize strict `/tasks` board claim/update + message loop and no orchestration commands.
...
**CRITICAL: Steps must execute in exact order. Never tear down the team handle before shutdown is confirmed.**

**Step 1: Verify completion**
```
Inspect the `/tasks` board — verify all real tasks (non-internal) are completed or failed.
```
...
**Step 4: Team teardown — only after ALL teammates confirmed or timed out**
```json
{ "team_name": "fix-ts-errors" }
```
...
Check for agent processes that survived team teardown:
```bash
node "${COPILOT_PLUGIN_ROOT}/scripts/cleanup-orphans.mjs" --team-name fix-ts-errors
```
...
**Shutdown sequence is BLOCKING:** Do not proceed to team teardown until all teammates have either:
...
The team skill supports **hybrid execution** combining Claude/GPT agent teammates with external CLI workers (Codex CLI and Gemini CLI). Both types can make code changes -- they differ in capabilities and cost. These are standalone CLI tools, not MCP servers.
...
|---------------|----------|-------------|
| `claude_worker` / `gpt_worker` | omcp agent | Full Copilot CLI tool access (Read/Write/Edit/Bash, `/fleet`). Best for tasks needing reasoning + iterative tool use. |
...
**Key difference from in-process teammates:**
- CLI workers operate via tmux, not Copilot CLI's tool system
- They cannot use the `/tasks` board or messaging APIs (no team awareness)
...
|-----------|-----------|-----|
| Iterative multi-step work | In-process teammate | Needs tool-mediated iteration + team communication |
...
| Build/test iteration loops | In-process teammate | Needs Bash tool + iterative fix cycles |
| Tasks needing team coordination | In-process teammate | Needs messaging for status updates |
...
The lead runs #1 (Codex security analysis), then #2 and #3 in parallel (Codex refactors backend, designer agent redesigns frontend), then #4 (in-process teammate handles test iteration), then #5 (Gemini final review).

### Pre-flight Analysis (Optional)

For large ambiguous tasks, run analysis before team creation:

1. Dispatch a subagent through `/fleet` targeting `planner` with task description + codebase context
...
The lead can proactively ingest outbox messages from CLI workers using the outbox reader utilities, enabling event-driven monitoring without relying solely on inbox delivery.
...
This approach complements the existing message-based communication by providing a pull-based mechanism for MCP workers that cannot use Copilot CLI's team messaging tools.

## Error Handling

### Teammate Fails a Task

1. Teammate sends a message to the lead reporting the failure
2. Lead decides: retry (reassign same task to same or different worker) or skip
3. To reassign: update the `/tasks` entry to set new owner, then message the new owner

### Teammate Gets Stuck (No Messages)

1. Lead detects via `/tasks` polling -- task stuck in `in_progress` for too long
2. Lead messages the teammate asking for status
3. If no response, consider the teammate dead
4. Reassign the task to another worker via `/tasks` update

### Dependency Blocked

1. If a blocking task fails, the lead must decide whether to:
   - Retry the blocker
   - Remove the dependency (update with modified blockedBy)
   - Skip the blocked task entirely
2. Communicate decisions to affected teammates via messages
...
4. If needed, dispatch a replacement teammate through `/fleet` (with team_name + name)
...
4. Inspect the `/tasks` board to determine current progress
...
| **Task claiming** | `/tasks` board update (owner + in_progress) -- lead pre-assigns | SQLite IMMEDIATE transaction -- atomic |
| **Race conditions** | Possible if two agents claim same task (mitigate by pre-assigning) | None (SQLite transactions) |
| **Communication** | Messaging API (DM, broadcast, shutdown) | None (fire-and-forget agents) |
| **Task dependencies** | Built-in `blocks` / `blockedBy` arrays | Not supported |
| **Heartbeat** | Automatic idle notifications from runtime | Manual heartbeat table + polling |
...
| **Progress visibility** | `/tasks` board shows live status with owner | SQL queries on tasks table |
...
| **State cleanup** | Team teardown removes everything | Manual `rm` of SQLite database |
...
Optional settings via `.omcp-config.json`:
...
- **monitorIntervalMs** - How often to poll `/tasks` (default: 30s)
- **shutdownTimeoutMs** - How long to wait for shutdown responses (default: 15s)

> **Note:** Team members do not have a hardcoded model default. Each teammate is a separate Copilot session that inherits the user's configured model (via `OMCP_MODEL_FAMILY` or `~/.copilot/config.json`). Since teammates can dispatch their own subagents, the session model acts as the orchestration layer while subagents can use any model tier.

## State Cleanup

On successful completion:

1. The team teardown call handles all runtime state:
...
**IMPORTANT:** Tear down the team handle only AFTER all teammates have been shut down. Team teardown will fail if active members (besides the lead) still exist in the config.
...
1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.omcp/worktrees/{team}/{worker}` with branch `omcp-team/{teamName}/{workerName}`.
...
1. **Internal tasks pollute the board** -- When a teammate is spawned, the system auto-creates an internal task with `metadata._internal: true`. These appear in the `/tasks` listing. Filter them when counting real task progress. The subject of an internal task is the teammate's name.

2. **No atomic claiming** -- Unlike SQLite swarm, there is no transactional guarantee on `/tasks` updates. Two teammates could race to claim the same task. **Mitigation:** The lead should pre-assign owners via `/tasks` update (taskId, owner) before spawning teammates. Teammates should only work on tasks assigned to them.

3. **Task IDs are strings** -- IDs are auto-incrementing strings ("1", "2", "3"), not integers. Always pass string values to `taskId` fields.

4. **Team teardown requires empty team** -- All teammates must be shut down before tearing down the team handle. The lead (the only remaining member) is excluded from this check.
...
11. **CLI workers are one-shot, not persistent** -- Tmux CLI workers have full filesystem access and CAN make code changes. However, they run as autonomous one-shot jobs -- they cannot use the `/tasks` board or messaging APIs. The lead must manage their lifecycle: write prompt_file, spawn CLI worker, read output_file, mark task complete. They don't participate in team communication like in-process teammates do.
