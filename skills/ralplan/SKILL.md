...
2. **User feedback** *(--interactive only)*: If `--interactive` is set, ask the user directly (one question at a time) to present the draft plan **plus the Principles / Drivers / Options summary** before review (Proceed to review / Request changes / Skip review). Otherwise, automatically proceed to review.
...
6. On Critic approval *(--interactive only)*: If `--interactive` is set, ask the user directly (one question at a time) to present the plan with approval options (Approve and implement via team (Recommended) / Approve and execute via ralph / Clear context and implement / Request changes / Reject). Final plan must include ADR (Decision, Drivers, Alternatives considered, Why chosen, Consequences, Follow-ups). Otherwise, output the final plan and stop.
7. *(--interactive only)* User chooses: Approve (team or ralph), Request changes, or Reject
8. *(--interactive only)* On approval: invoke `/oh-my-copilot:team` for parallel team execution (recommended) or `/oh-my-copilot:ralph` for sequential execution -- never implement directly

> **Important:** Steps 3 and 4 MUST run sequentially. Do NOT dispatch both Architect and Critic in the same parallel `/fleet` batch. Always await the Architect result before dispatching the Critic.
