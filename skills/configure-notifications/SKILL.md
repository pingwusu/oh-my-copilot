...
- If no provider is specified, ask the user directly (one question at a time):
...
This is an interactive, natural-language configuration skill. Walk the user through setup by asking one question at a time. Write the result to `~/.copilot/.omcp-config.json`.
...
4. Choose a username (e.g., "my_omcp_bot")
...
The user will type their token.
...
The user will type their chat ID.
...
Ask the user directly (one question at a time) with multi-select semantics:
...
Env vars are auto-detected by the notification system without needing `.omcp-config.json`.
...
The user will type their webhook URL.
...
Build the notifications object with the collected values and merge into `.omcp-config.json` using jq:
...
- `omcp --telegram` — Activates Telegram notifications (sets `OMCP_TELEGRAM=1`)
- `omcp --discord` — Activates Discord notifications (sets `OMCP_DISCORD=1`)
- `omcp --slack` — Activates Slack notifications (sets `OMCP_SLACK=1`)
- `omcp --webhook` — Activates webhook notifications (sets `OMCP_WEBHOOK=1`)
- `omcp --openclaw` — Activates OpenClaw gateway integration (sets `OMCP_OPENCLAW=1`)
...
**Examples:**
- `omcp --telegram --discord` — Telegram + Discord active
- `omcp --telegram --slack --webhook` — Telegram + Slack + Webhook active
- `omcp --telegram --openclaw` — Telegram + OpenClaw active
- `omcp` — No notifications sent (all platforms require explicit activation)

---

## Hook Event Templates

Customize notification messages per event and per platform using `omcp_config.hook.json`.
...
Check if `~/.copilot/omcp_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Read or create `~/.copilot/omcp_config.hook.json` and merge the new settings:
...
```bash
LEGACY_CONFIG="$HOME/.copilot/omcp_config.openclaw.json"
...
If migrate:
- Read `omcp_config.openclaw.json`
- Transform to custom integration format
- Save to `.omcp-config.json`
- Backup legacy to `omcp_config.openclaw.json.bak`
