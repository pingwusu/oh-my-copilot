...
[RALPH + PARALLEL DISPATCH - ITERATION {{ITERATION}}/{{MAX}}]
...
- User wants manual control over completion -- dispatch executors through `/fleet` directly
</Do_Not_Use_When>
...
- Always pass the `model` parameter explicitly when dispatching agents through `/fleet`
...
3. **Implement the current story**:
   - Dispatch specialist subagents through `/fleet` at appropriate tiers:
     - Simple lookups: LOW tier (Haiku / GPT-5-mini) -- "What does this function return?"
     - Standard work: MEDIUM tier (Sonnet / GPT-5.2-codex) -- "Add error handling to this module"
     - Complex analysis: HIGH tier (Opus / GPT-5.4) -- "Debug this race condition"
...
7. **Reviewer verification** (tiered, against acceptance criteria):
   - <5 files, <100 lines with full tests: STANDARD tier minimum (architect-medium / Sonnet / GPT-5.2-codex)
   - Standard changes: STANDARD tier (architect-medium / Sonnet / GPT-5.2-codex)
   - >20 files or security/architectural changes: THOROUGH tier (architect / Opus / GPT-5.4)
   - If `--critic=critic`, use the `critic` agent (dispatched through `/fleet`) for the approval pass
   - If `--critic=codex`, run `omcp ask codex --agent-prompt critic "..."` for the approval pass
...
7.5 **Mandatory Deslop Pass**:
   - Unless `{{PROMPT}}` contains `--no-deslop`, run `oh-my-copilot:ai-slop-cleaner` in standard mode (not `--review`) on the files changed during the current Ralph session only.
...
<Good>
Correct parallel delegation (three subagents dispatched through `/fleet` simultaneously):
```
/fleet executor --model=haiku --prompt="Add type export for UserConfig"
/fleet executor --model=sonnet --prompt="Implement the caching layer for API responses"
/fleet executor --model=opus --prompt="Refactor auth module to support OAuth2 flow"
```
...
/fleet executor "Implement caching" → wait →
/fleet executor "Refactor auth"
```
...
- [ ] Zero pending or in_progress `/tasks` items
