...
Examples:
  ...
  psm feature omcp add-webhooks
  psm list
  psm attach omcp:pr-123
  psm kill omcp:pr-123
  psm cleanup
EOF
}
    ...
    if [[ "$no_tmux" != "true" ]]; then
        ...
        if [[ "$tmux_status" == "error" ]]; then
            ...
            if [[ "$no_copilot" != "true" ]]; then
                ...
                psm_launch_copilot "$session_name"
            fi
        fi
    fi
