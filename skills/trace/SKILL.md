---
name: trace
description: Evidence-driven tracing lane that orchestrates competing tracer hypotheses in Copilot built-in team mode
agent: tracer
level: 2
model:
...
Use **Copilot's built-in team mode** (via `/fleet` parallel dispatch, or `omcp team` for explicit multi-agent runs) for `/trace`.
...
4. Spawn **3 tracer lanes by default** through `/fleet`
...
- existing trace artifacts via trace timeline tooling
- existing aggregate trace evidence via trace summary tooling
...
3. "Create one tracer lane per hypothesis by dispatching three workers through `/fleet`."
