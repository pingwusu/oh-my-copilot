...
Dispatch a subagent through `/fleet` to enumerate directories:

```
/fleet explore --model=haiku --prompt="List all directories recursively. Exclude: node_modules, .git, dist, build, __pycache__, .venv, coverage, .next, .nuxt"
```

### Step 2: Create Work Plan

Generate `/tasks` entries for each directory, organized by depth level:
...
| Task | Agent (dispatched via `/fleet`) |
|------|---------------------------------|
...
1. **Same-level directories**: Process in parallel by dispatching multiple `/fleet` workers
