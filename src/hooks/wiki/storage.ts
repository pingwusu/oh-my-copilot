 ...
 *
 * Storage layout (default — see resolveWikiRoot for overrides):
 *   <root>/.omcp/wiki/
 ...
 *   └── ...
 *
 * Roots can be overridden by `OMCP_WIKI_ROOT` (absolute path to the `.omcp/`
 * parent — i.e. the project root). This is what tests set.
 */
...
/**
 * Resolve the `.omcp/` root for wiki storage.
 *
 * Precedence:
 *   1. explicit `root` argument joined with `.omcp`
 *   2. `OMCP_WIKI_ROOT` env var (treated as an absolute path to `.omcp`)
 *   3. `<root>/.omcp/`
 */
function getOmcpRoot(root: string): string {
...
export function getWikiDir(root: string): string {
  return join(getOmcpRoot(root), WIKI_DIR);
}
...
export function ensureWikiDir(root: string): string {
  ...
  const omcpRoot = getOmcpRoot(root);
...
// ============================================================================
// Atomic write (sync) — inlined from upstream lib/atomic-write.ts
// ============================================================================
...
function atomicWriteFileSync(filePath: string, content: string): void {
  ...
  // Crypto.randomUUID requires importing; use a cheap unique suffix instead.
  const tempPath = `${filePath}.tmp.${process.pid}.${Date.now()}.${Math.floor(
    Math.random() * 1e9,
  )}`;
...
// ============================================================================
// File lock (sync) — minimal in-process+cross-process lock
// ============================================================================
...
function tryAcquireLockSync(lockPath: string): number | null {
...
function acquireLockSync(
  ...
  let fd = tryAcquireLockSync(lockPath);
  if (fd !== null) return fd;
  while (Date.now() < deadline) {
    // Brief spin since we are in sync context.
    const waitUntil = Date.now() + Math.min(retryDelayMs, deadline - Date.now());
    while (Date.now() < waitUntil) {
      /* spin */
    }
    fd = tryAcquireLockSync(lockPath);
    if (fd !== null) return fd;
  }
  throw new Error(`Failed to acquire wiki lock: ${lockPath}`);
}
...
export function withWikiLock<T>(root: string, fn: () => T): T {
  ...
  const lockPath = join(wikiDir, ".wiki.lock");
  const fd = acquireLockSync(lockPath, 5_000, 50);
  try {
    return fn();
  } finally {
    releaseLockSync(fd, lockPath);
  }
}
