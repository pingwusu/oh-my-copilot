...
export function lintWiki(
  ...
  appendLog(root, {
    ...
    summary:
      ...
      `(${stats.orphanCount} orphan, ${stats.staleCount} stale, ` +
      `${stats.brokenRefCount} broken, ${stats.contradictionCount} contradictions)`,
  });
    ...
    for (const [tag, categories] of tagCategoryPairs) {
      if (categories.size > 1) {
        issues.push({
          ...
          severity: "info",
          type: "structural-contradiction",
          message: `Tag "${tag}" appears in pages with different categories: ${[
            ...categories,
          ].join(", ")}`,
        });
        break;
